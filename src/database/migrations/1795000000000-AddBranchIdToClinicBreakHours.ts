...
export class AddBranchIdToClinicBreakHours1795000000000
  implements MigrationInterface
{
  public async up(queryRunner: QueryRunner): Promise<void> {
    let table = await queryRunner.getTable('clinic_break_hours');
    const hasBranchIdColumn = table?.findColumnByName('branch_id');

    // Add branch_id column to clinic_break_hours table if it doesn't exist
    if (!hasBranchIdColumn) {
      ...
      table = await queryRunner.getTable('clinic_break_hours');
    }
    ...
    const existingIndexes = table?.indices.filter(
      (idx) => idx.name === 'IDX_clinic_break_hours_branch',
    );
    ...
    if (!hasBranchIndex) {
      await queryRunner.createIndex(
        'clinic_break_hours',
        new TableIndex({
          name: 'IDX_clinic_break_hours_branch',
          columnNames: ['branch_id'],
        }),
      );
    ...
    const existingCompositeIndexes = table?.indices.filter(
      (idx) => idx.name === 'IDX_clinic_break_hours_clinic_day_branch',
    );
    ...
    // Keep the existing IDX_clinic_break_hours_clinic_day index as it may be used by foreign key constraints
    if (!hasCompositeIndex) {
      await queryRunner.createIndex(
        'clinic_break_hours',
        new TableIndex({
          name: 'IDX_clinic_break_hours_clinic_day_branch',
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    const table = await queryRunner.getTable('clinic_break_hours');
    ...
    if (existingCompositeIndexes && existingCompositeIndexes.length > 0) {
      await queryRunner.dropIndex(
        'clinic_break_hours',
        'IDX_clinic_break_hours_clinic_day_branch',
      );
    }
    ...
    const hasBranchIdColumn = table?.findColumnByName('branch_id');
    if (hasBranchIdColumn) {
      await queryRunner.dropColumn('clinic_break_hours', 'branch_id');
    }
  }
}
